        ...
        private static void WriteMessagesFromList(string sFilePath)
        {
            ...
            using (StreamWriter swLogWriter = File.AppendText(sFilePath))
            {
                if (swLogWriter != null)
                {
                    ...
                    {
                        //
                        // Change message format based on whether or not we are writing an exception
                        //
                        string sMessageFormat = logObject.IsException ? LOG_MESSAGE_EXCEPTION_FORMAT : LOG_MESSAGE_REGULAR_FORMAT;

                        if (logObject != null)
                        {
                            string sLine = string.Format(sMessageFormat,
